...
/**
 * This class, Mollie_SplitOrder, handles the splitting of payments for Mollie payments in WooCommerce.
 * It calculates the routing information for payments based on vendor sales and shipping amounts.
 */
...
class Mollie_SplitOrder {
    ...
    public static function getOrderRouting($order) {
		...
		$logger->debug( 'BHW getOrderRouting Order: ' . print_r($order,true), $context );
                ...
                if( !is_null($organizationId) ) { 
                    ...
                    if(  $gift_cards_totals == 0 ){
                            ...
                            $routes[] = [
                                "amount" => [                    
                                    "currency" => $currency,
                                    "value" => self::mollieWooCommerceFormatCurrencyValue( $amount, $currency)
                                ],
                                "destination" => [
                                    "type" => "organization",
    ...
    public static function applyBluehouseFeeToVendor($vendor_total) {    
        ...
        //$fee = 10; //get_option('mollie-payments-for-woocommerce_bluehouse_fee_percent');
	    $fee = get_option('bluehouse_fee_percent', 10);
        $logger = wc_get_logger();
		$context = array( 'source' => 'bhw-split-payment-for-mollie' );
        $logger->debug( 'applyBluehouseFeeToVendor : '. print_r($fee,true) , $context );

        if($fee) {
            $vendor_total -= $vendor_total * (1/$fee);
        }

        $logger->debug( 'vendor_total : '. print_r($vendor_total,true) , $context );
    ...
    public static function getReleaseDate() {
        ...
        //  $releaseDays = 30;
        $releaseDays = get_option('payment_release_days', 30);
	    $logger = wc_get_logger();
        $context = array( 'source' => 'bhw-split-payment-for-mollie' );
        $logger->debug( 'getReleaseDate : '. print_r($releaseDays,true) , $context );

        if($releaseDays && $releaseDays > 0) {
            $releaseDate = new DateTime();
            $releaseDate->add(new DateInterval("P{$releaseDays}D"));
            $logger->debug( 'getReleaseDate Format : '. print_r($releaseDate->format('Y-m-d'),true) , $context );
    ...
    public static function mollieWooCommerceFormatCurrencyValue($value, $currency)
    {
